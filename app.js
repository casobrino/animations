...
window.addEventListener("scroll", () => {
  ...
  saluteTextContainer.style.transform = `translateY(${-offsetY * 0.8}px)`;
  saluteImgContainer.style.transform = `translate(${offsetY * 0.2}px, ${
    -offsetY * 0.3
  }px)`;
  authorName.style.transform = `translateX(calc(100vh - ${offsetY * 1}px))`;
  jobTitleContainer[0].style.backgroundPositionY = `${offsetY * 0.5}px`;
  jobTitleContainer[1].style.backgroundPositionY = `${-offsetY * 0.5}px`;
  jobTitle[0].style.transform = `translateX(calc(200vh - ${offsetY * 1}px))`;
  jobTitle[1].style.transform = `translateX(calc(-300vh + ${offsetY * 1}px))`;
